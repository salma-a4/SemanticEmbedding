...
public class HeapSort {
    ...
    private static void heapSort(List<CosSimilarityPair> _list) {
        ...
        for (int j = n - 1; j > 0; j--) {
            CosSimilarityPair temp = new CosSimilarityPair(_list.get(0).getWord1(), _list.get(0).getWord2(), _list.get(0).getCosineSimilarity());
    ...
    private static void heapify(List<CosSimilarityPair> _tree, int _rootindex, int _n) {
        ...
        while (2 * _rootindex + 2 <= _n) {
            ...
            //Swap the root with the largest node.
            CosSimilarityPair temp = new CosSimilarityPair(_tree.get(_rootindex).getWord1(), _tree.get(_rootindex).getWord2(), _tree.get(_rootindex).getCosineSimilarity());
